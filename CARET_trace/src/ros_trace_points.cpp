...
void ros_trace_rcl_node_init(
  const void * node_handle, const void * rmw_handle, const char * node_name,
  const char * node_namespace)
{
  ...
  if (controller.is_allowed_node(node_handle)) {
    ...
    ((functionT)ORIG_FUNC::ros_trace_rcl_node_init)(
      node_handle, rmw_handle, node_name, node_namespace);

#ifdef DEBUG_OUTPUT
    std::cerr << "rcl_node_init," << node_handle << "," << rmw_handle << "," << node_name << ","
...
void ros_trace_rcl_subscription_init(
  const void * subscription_handle, const void * node_handle, const void * rmw_subscription_handle,
  const char * topic_name, const size_t queue_depth)
{
  static auto & controller = Singleton<TracingController>::get_instance();
  ...
  if (controller.is_allowed_subscription_handle(subscription_handle)) {
    ...
    ((functionT)ORIG_FUNC::ros_trace_rcl_subscription_init)(
      subscription_handle, node_handle, rmw_subscription_handle, topic_name, queue_depth);
#ifdef DEBUG_OUTPUT
    std::cerr << "rcl_subscription_init," << subscription_handle << "," << node_handle << ","
              << rmw_subscription_handle << "," << topic_name << "," << queue_depth << std::endl;
#endif
  }
}
...
#ifdef DEBUG_OUTPUT
    std::cerr << "rclcpp_subscription_init," << subscription_handle << "," << subscription
              << std::endl;
#endif
  }
}
...
#ifdef DEBUG_OUTPUT
    std::cerr << "rclcpp_subscription_callback_added," << subscription << "," << callback
              << std::endl;
#endif
  }
}
...
#ifdef DEBUG_OUTPUT
    std::cerr << "rclcpp_timer_callback_added," << timer_handle << "," << callback << std::endl;
#endif
  }
}
...
#ifdef DEBUG_OUTPUT
    std::cerr << "rclcpp_timer_link_node," << timer_handle << "," << node_handle << std::endl;
#endif
  }
}
...
#ifdef DEBUG_OUTPUT
    std::cerr << "callback_start," << callback << "," << is_intra_process << std::endl;
#endif
  }
}
...
#ifdef DEBUG_OUTPUT
    std::cerr << "callback_end," << callback << std::endl;
#endif
  }
}

void ros_trace_dispatch_subscription_callback(
  const void * message, const void * callback, const uint64_t source_timestamp,
  const uint64_t message_timestamp)
{
  ...
  if (controller.is_allowed_callback(callback)) {
    ...
    ((functionT)ORIG_FUNC::ros_trace_dispatch_subscription_callback)(
      message, callback, source_timestamp, message_timestamp);

#ifdef DEBUG_OUTPUT
    std::cerr << "dispatch_subscription_callback," << message << "," << callback << ","
              << source_timestamp << "," << message_timestamp << std::endl;
#endif
  }
}

void ros_trace_dispatch_intra_process_subscription_callback(
  const void * message, const void * callback, const uint64_t message_timestamp)
{
  static auto & controller = Singleton<TracingController>::get_instance();
  ...
  if (controller.is_allowed_callback(callback)) {
    ...
    ((functionT)ORIG_FUNC::ros_trace_dispatch_intra_process_subscription_callback)(
      message, callback, message_timestamp);

#ifdef DEBUG_OUTPUT
    std::cerr << "dispatch_intra_process_subscription_callback," << message << "," << callback
...
void ros_trace_rclcpp_publish(
  const void * publisher_handle, const void * message, const uint64_t message_timestamp)
{
  static auto & controller = Singleton<TracingController>::get_instance();
...
#ifdef DEBUG_OUTPUT
    std::cerr << "rclcpp_publish," << publisher_handle << "," << message << "," << message_timestamp
              << std::endl;
#endif
  }
}
  ...
  const void * publisher_handle, const void * message, const uint64_t message_timestamp)
{
  ...
  if (controller.is_allowed_publisher_handle(publisher_handle)) {
    ...
    ((functionT)ORIG_FUNC::ros_trace_rclcpp_intra_publish)(
      publisher_handle, message, message_timestamp);
#ifdef DEBUG_OUTPUT
    std::cerr << "rclcpp_intra_publish," << publisher_handle << "," << message << ","
              << message_timestamp << std::endl;
#endif
  }
}
...
void ros_trace_rcl_timer_init(const void * timer_handle, int64_t period)
{
  ...
  std::cerr << "rcl_timer_init," << timer_handle << "," << period << std::endl;
}
#endif
...
void ros_trace_rcl_init(const void * context_handle)
{
  ...
  std::cerr << "rcl_init," << context_handle << std::endl;
}
#endif
...
void ros_trace_rcl_publisher_init(
  const void * publisher_handle, const void * node_handle, const void * rmw_publisher_handle,
  const char * topic_name, const size_t queue_depth)
{
  static auto & controller = Singleton<TracingController>::get_instance();
  ...
  ((functionT)ORIG_FUNC::ros_trace_rcl_publisher_init)(
    publisher_handle, node_handle, rmw_publisher_handle, topic_name, queue_depth);
  std::cerr << "rcl_publisher_init," << publisher_handle << "," << node_handle << ","
            << rmw_publisher_handle << "," << topic_name << "," << queue_depth << std::endl;
}
#endif
...
#ifdef DEBUG_OUTPUT
    std::cerr << "rcl_publish," << publisher_handle << "," << message << std::endl;
#endif
  }
}
...
void ros_trace_rcl_service_init(
  const void * service_handle, const void * node_handle, const void * rmw_service_handle,
  const char * service_name)
{
  using functionT = void (*)(const void *, const void *, const void *, const char *);
  assert(ORIG_FUNC::ros_trace_rcl_service_init != nullptr);
  ((functionT)ORIG_FUNC::ros_trace_rcl_service_init)(
    service_handle, node_handle, rmw_service_handle, service_name);

  std::cerr << "rcl_service_init," << service_handle << "," << node_handle << ","
            << rmw_service_handle << "," << service_name << std::endl;
}
#endif

#ifdef DEBUG_OUTPUT
void ros_trace_rclcpp_service_callback_added(const void * service_handle, const char * callback)
{
  using functionT = void (*)(const void *, const void *);
  ...
  std::cerr << "rclcpp_service_callback_added," << service_handle << "," << callback << std::endl;
}
#endif

#ifdef DEBUG_OUTPUT
void ros_trace_rcl_client_init(
  const void * client_handle, const void * node_handle, const void * rmw_client_handle,
  const char * service_name)
{
  using functionT = void (*)(const void *, const void *, const void *, const char *);
  assert(ORIG_FUNC::ros_trace_rcl_client_init != nullptr);
  ((functionT)ORIG_FUNC::ros_trace_rcl_client_init)(
    client_handle, node_handle, rmw_client_handle, service_name);

  std::cerr << "rcl_client_init," << client_handle << "," << node_handle << "," << rmw_client_handle
            << "," << service_name << std::endl;
}
#endif

void ros_trace_rclcpp_callback_register(const void * callback, const char * symbol)
{
  static auto & controller = Singleton<TracingController>::get_instance();
...
#ifdef DEBUG_OUTPUT
    std::cerr << "rclcpp_callback_register," << callback << "," << symbol << std::endl;
#endif
  }
}
...
void ros_trace_rcl_lifecycle_state_machine_init(
  const void * node_handle, const void * state_machine)
{
  using functionT = void (*)(const void *, const void *);
  ...
  std::cerr << "rcl_lifecycle_state_machine_init," << node_handle << "," << state_machine
            << std::endl;
}
#endif
  ...
  const void * state_machine, const char * start_label, const char * goal_label)
{
  ...
  ((functionT)ORIG_FUNC::ros_trace_rcl_lifecycle_transition)(
    state_machine, start_label, goal_label);

  std::cerr << "rcl_lifecycle_transition," << state_machine << "," << start_label << ","
            << goal_label << "," << std::endl;
}
#endif

#ifdef DEBUG_OUTPUT
void ros_trace_message_construct(const void * original_message, const void * constructed_message)
{
  using functionT = void (*)(const void *, const void *);

  std::cerr << "message_construct," << original_message << "," << constructed_message << std::endl;
}
#endif

void ros_trace_rclcpp_executor_execute(const void * handle)
{
  // #ifdef DEBUG_OUTPUT
...
void ros_trace_rclcpp_executor_wait_for_work(const int64_t timeout)
{
  // #ifdef DEBUG_OUTPUT
...
void ros_trace_rcl_take(const void * message)
{
  // #ifdef DEBUG_OUTPUT
...
void ros_trace_rclcpp_take(const void * message) {}

void ros_trace_rmw_take(
  const void * rmw_subscription_handle, const void * message, int64_t source_timestamp,
  const bool taken)
{
  // #ifdef DEBUG_OUTPUT
  //   std::cerr << "rmw_take," <<
...
#ifdef DEBUG_OUTPUT
  std::cerr << "rmw_publish," << message << "," << std::endl;
#endif
}

void ros_trace_rmw_publisher_init(const void * rmw_publisher_handle, const uint8_t gid)
{
  // #ifdef DEBUG_OUTPUT
...
void ros_trace_rmw_subscription_init(const void * rmw_subscription_handle, const uint8_t gid)
{
  // #ifdef DEBUG_OUTPUT
